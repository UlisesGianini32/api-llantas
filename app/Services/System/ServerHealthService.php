...
final class ServerHealthService
{
    ...
    /**
     * Genera una fotografía completa del estado actual del servidor.
     *
    ...
    private function swap(): array
    {
        ...
        $totalKb = $memInfo['SwapTotal'] ?? 0;
        $freeKb = $memInfo['SwapFree'] ?? 0;
    ...
    private function disk(): array
    {
        try {
            ...
            if ($total === false || $free === false || $total <= 0) {
                throw new \RuntimeException('No fue posible consultar el espacio del disco.');
            }
    ...
    private function loadAverage(): array
    {
        ...
        $normalized = round(($values[0] / $cores) * 100, 2);
    ...
    private function database(): array
    {
        ...
        try {
            ...
            try {
                $versionResult = $connection->selectOne('SELECT VERSION() AS version');
                $version = $versionResult?->version ?? null;
            } catch (Throwable) {
                // La conexión sigue siendo válida aunque no se pueda leer la versión.
            }
            ...
            return [
                ...
                'server_version' => $version ?: $pdo->getAttribute(\PDO::ATTR_SERVER_VERSION),
                'response_time_ms' => round((microtime(true) - $startedAt) * 1000, 2),
            ];
        } catch (Throwable $exception) {
            return [
    ...
    private function scheduler(): array
    {
        ...
        foreach ($keys as $key) {
            try {
                ...
                // Continuamos con la siguiente alternativa.
            }
        }
        ...
        try {
            $lastRun = match (true) {
                $value instanceof \DateTimeInterface => \Illuminate\Support\Carbon::instance($value),
                is_numeric($value) => \Illuminate\Support\Carbon::createFromTimestamp((int) $value),
                default => \Illuminate\Support\Carbon::parse((string) $value),
            };
    ...
    private function overallStatus(array $components): array
    {
        ...
        $status = match (true) {
            ...
            $statuses->every(fn (string $value): bool => $value === self::STATUS_UNKNOWN) => self::STATUS_UNKNOWN,
            default => self::STATUS_HEALTHY,
        };
        ...
        return [
            ...
            'healthy_count' => $statuses->filter(
                fn (string $value): bool => $value === self::STATUS_HEALTHY
            )->count(),
            'warning_count' => $statuses->filter(
                fn (string $value): bool => $value === self::STATUS_WARNING
            )->count(),
            'critical_count' => $statuses->filter(
                fn (string $value): bool => $value === self::STATUS_CRITICAL
            )->count(),
            'unknown_count' => $statuses->filter(
    ...
    private function cpuUsagePercent(): ?float
    {
        ...
        return round(
            max(0, min(100, (1 - ($idleDelta / $totalDelta)) * 100)),
            2
        );
    }
    ...
    private function cpuModel(): ?string
    {
        ...
        if (preg_match('/^model name\s*:\s*(.+)$/m', $contents, $matches)) {
            return trim($matches[1]);
        }

        if (preg_match('/^Hardware\s*:\s*(.+)$/m', $contents, $matches)) {
            return trim($matches[1]);
        }
    ...
    private function processList(): ?array
    {
        ...
        @exec('ps -eo pid=,args= 2>/dev/null', $output, $exitCode);
