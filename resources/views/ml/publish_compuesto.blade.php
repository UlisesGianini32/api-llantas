<x-layouts.app title="Publicar compuesto en MercadoLibre">

<div class="mx-auto max-w-6xl space-y-6">

    <div class="flex items-start justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-zinc-900 dark:text-white">Publicar compuesto en MercadoLibre</h1>

            <div class="text-sm text-zinc-500 dark:text-neutral-400 mt-1">
                SKU compuesto: <span class="font-mono">{{ $compuesto->sku }}</span>
                · Paquete: <b>{{ $packQty }}</b>
                · Precio: <b>${{ number_format((float)($compuesto->precio_ML ?? $compuesto->precio_ml ?? $compuesto->precio ?? 0), 2) }}</b>
                · Stock actual: <b>{{ (int)($compuesto->stock ?? 0) }}</b>
            </div>

            <div class="text-xs text-zinc-500 dark:text-neutral-400 mt-1">
                Base: <b>{{ $baseLlanta->marca ?? '—' }}</b>
                · <b>{{ $baseLlanta->medida ?? '—' }}</b>
                · SKU base: <span class="font-mono">{{ $baseLlanta->sku ?? '—' }}</span>
            </div>
        </div>
    ...
    <div class="rounded-2xl border border-zinc-200 bg-white dark:bg-neutral-900 dark:border-neutral-800 overflow-hidden">
        ...
        <form id="publishForm"
              method="POST"
              action="{{ route('productos.ml.publish', $compuesto->id) }}"
            ...
            <section class="step-pane p-6 space-y-6" data-step-pane="1">
                ...
                <div class="rounded-2xl border border-zinc-200 bg-white p-4 dark:bg-neutral-950 dark:border-neutral-800">
                    ...
                    <div id="catCatalogHint" class="mt-3 hidden p-3 rounded-lg border text-sm"></div>
                ...
                <div class="rounded-2xl border border-zinc-200 bg-white p-4 dark:bg-neutral-950 dark:border-neutral-800">
                    ...
                    <div class="text-[11px] text-zinc-500 dark:text-neutral-400 mt-1">
                        Si la categoría usa catálogo, puedes elegir un producto. Si no lo encuentras, podrás intentar publicar separado.
                    </div>
                    ...
                    <div class="mt-4">
                        ...
                        <div class="flex gap-2 mt-2">
                            <input id="catalogQuery"
                                   type="text"
                                   value="{{ old('catalogQuery', trim(($baseLlanta->marca ?? '').' '.($baseLlanta->medida ?? '').' '.($baseLlanta->title_familyname ?? ''))) }}"
            ...
            <section class="step-pane hidden p-6 space-y-6" data-step-pane="2">
                ...
                <div>
                    ...
                    <p class="text-sm text-zinc-500 dark:text-neutral-400 mt-1">
                        Este compuesto se publicará como paquete de <b>{{ $packQty }}</b> llantas.
                    </p>
                </div>
                ...
                <div class="rounded-2xl border border-zinc-200 bg-white p-4 dark:bg-neutral-950 dark:border-neutral-800">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                        <div>
                            ...
                            <input type="text"
                                   name="brand"
                                   value="{{ old('brand', $baseLlanta->marca ?? ($compuesto->llanta->marca ?? '')) }}"
                                   class="mt-1 w-full rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm dark:border-neutral-700 dark:bg-neutral-950 dark:text-white" />
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text"
                                   name="load_index"
                                   value="{{ old('load_index') }}"
                                   placeholder="Ej: 125/122"
                                   class="mt-1 w-full rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm dark:border-neutral-700 dark:bg-neutral-950 dark:text-white" />
                        </div>

                        <div>
                            <label class="text-xs font-semibold text-zinc-600 dark:text-neutral-300">Cantidad de llantas (fijo)</label>
                            <input type="number"
                                   min="1"
                                   max="20"
                                   name="tire_quantity"
                                   value="{{ old('tire_quantity', $packQty) }}"
                                   readonly
                                   class="mt-1 w-full rounded-lg border border-zinc-300 bg-zinc-50 px-3 py-2 text-sm dark:border-neutral-700 dark:bg-neutral-800 dark:text-white" />
                        </div>
                ...
                <div class="rounded-2xl border border-zinc-200 bg-white p-4 dark:bg-neutral-950 dark:border-neutral-800">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                        ...
                        <div>
                            <label class="text-xs font-semibold text-zinc-600 dark:text-neutral-300">Stock a publicar (del compuesto)</label>
                            <input type="number"
                                   min="1"
                                   max="9999"
                                   name="stock_input"
                                   value="{{ old('stock_input', max(1, (int)($compuesto->stock ?? 1))) }}"
                                   class="mt-1 w-full rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm dark:border-neutral-700 dark:bg-neutral-950 dark:text-white" />
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text"
                                   name="gtin"
                                   value="{{ old('gtin') }}"
                                   placeholder="Opcional (solo si lo tienes)"
                                   class="mt-1 w-full rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm dark:border-neutral-700 dark:bg-neutral-950 dark:text-white" />
                        </div>

                        <div>
                            <label class="text-xs font-semibold text-zinc-600 dark:text-neutral-300">SKU (del compuesto)</label>
                            <input type="text"
                                   name="seller_sku"
                                   value="{{ old('seller_sku', $compuesto->sku) }}"
                                   class="mt-1 w-full rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm dark:border-neutral-700 dark:bg-neutral-950 dark:text-white" />
                        </div>
                    </div>
                ...
                <div class="rounded-2xl border border-zinc-200 bg-white p-4 dark:bg-neutral-950 dark:border-neutral-800">
                    <div class="text-sm font-semibold text-zinc-900 dark:text-white">Título del pack</div>
                    <div class="text-[11px] text-zinc-500 dark:text-neutral-400 mt-1">
                        Debe quedar diferente al de la llanta individual. Ejemplo: <b>Pack de {{ $packQty }} llantas ...</b>
                    </div>
