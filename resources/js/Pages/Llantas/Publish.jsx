...
/** Enlace público a la publicación (sitio MLM / México). */
...
/** Respuesta ML domain_discovery usa category_id / category_name; el backend normaliza a id/name, pero dejamos fallback. */
...
export default function Publish() {
    const { llanta, errors: pageErrors = {}, flash = {} } = usePage().props
    ...
    const [catQuery, setCatQuery] = useState(
        [llanta?.marca, llanta?.medida, llanta?.title_familyname].filter(Boolean).join(' ').trim()
    )
    ...
    const [catalogQuery, setCatalogQuery] = useState(
        [llanta?.marca, llanta?.medida, llanta?.title_familyname]
            .filter(Boolean)
            .join(' ')
            .trim()
    )
    ...
    const form = useForm({
        ...
        title: llanta?.title_familyname
            ? llanta.title_familyname
            : `LLANTA ${llanta?.medida ?? ''} ${llanta?.marca ?? ''}`.trim(),
        ...
        speed_rating: '',
        stock_input: Math.max(1, Number(llanta?.stock ?? 1)),
        gtin: '',
        seller_sku: llanta?.sku ?? '',
    ...
    const submit = (e) => {
        ...
        form.post(`/llantas/${llanta.id}/ml/publish`, {
            forceFormData: true,
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-zinc-500 dark:text-neutral-400">
                            Flujo en tres pasos, alineado al asistente de publicación de Mercado Libre.
                        </p>
                        ...
                        <div className="mt-3 flex flex-wrap gap-2">
                            <span className="inline-flex items-center gap-2 rounded-full border border-zinc-200 bg-zinc-100 px-3 py-1 text-xs font-medium text-zinc-800 dark:border-neutral-600 dark:bg-neutral-800 dark:text-zinc-100">
                                ...
                                <span className="font-mono text-zinc-900 dark:text-white">{llanta.sku}</span>
                            </span>
                            <span className="inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-xs font-medium text-emerald-900 dark:border-emerald-900 dark:bg-emerald-950/60 dark:text-emerald-100">
                                <span className="text-emerald-700/80 dark:text-emerald-300">Precio ML</span>
                                <span>
                                    $
                                    {Number(llanta.precio_ML ?? 0).toLocaleString('es-MX', {
                                        minimumFractionDigits: 2,
                                        maximumFractionDigits: 2,
                                    })}
                                </span>
                            </span>
                            <span className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-medium text-zinc-800 dark:border-neutral-600 dark:bg-neutral-900 dark:text-zinc-100">
                                <span className="text-zinc-500 dark:text-zinc-400">Stock</span>
                                <span className="font-semibold">{llanta.stock}</span>
                            </span>
                        </div>
                    </div>
                            ...
                            <button
                                ...
                                className={`rounded-xl px-4 py-2.5 text-sm font-semibold transition ${
                                    step === 1
                                        ...
                                        : 'border border-zinc-300 text-zinc-700 hover:bg-zinc-50 dark:border-neutral-600 dark:text-zinc-200 dark:hover:bg-neutral-800'
                                }`}
                            >
                    ...
                    <form onSubmit={submit} className="space-y-0">
                            ...
                            <section className="space-y-6 p-6">
                                ...
                                <div className="rounded-2xl border border-zinc-200 bg-white p-4 dark:border-neutral-800 dark:bg-neutral-950">
                                    ...
                                    <p className="mt-1 text-xs text-zinc-500 dark:text-neutral-400">
                                        Usa marca, medida o palabras como «llanta», «R18», etc.
                                    </p>
                                    ...
                                    {selectedCategory && (
                                        <div className="mt-3 rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-emerald-800 dark:border-emerald-900 dark:bg-emerald-950 dark:text-emerald-200">
                                            ...
                                            {' · '}ID:{' '}
                                            <span className="font-mono font-bold">{selectedCategory.id}</span>
                                        </div>
                                    )}
                                ...
                                <div className="rounded-2xl border border-zinc-200 bg-white p-4 dark:border-neutral-800 dark:bg-neutral-950">
                                    ...
                                    <div className="mt-1 text-xs text-zinc-500 dark:text-neutral-400">
                                        Si la categoría usa catálogo, elige un producto. Si no aplica o no encuentras match, usa los botones de abajo.
                                    </div>
                                    ...
                                    <div className="mt-4">
                                        ...
                                        <div className="mt-3 text-sm">
                                            {catalogResults.length === 0 ? (
                                                <div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-600 dark:border-neutral-700 dark:bg-neutral-900/60 dark:text-slate-300">
                                                    Primero confirma una categoría arriba; luego busca aquí el producto del catálogo.
                                                </div>
                                            ) : (
                                                <div className="space-y-2">
                                        ...
                                        {selectedCatalog && (
                                            <div className="mt-3 rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-emerald-800 dark:border-emerald-900 dark:bg-emerald-950 dark:text-emerald-200">
                                                ...
                                                <span className="font-semibold">{selectedCatalog.id}</span>
                                            </div>
                                        )}

                                        {form.data.catalog_mode === 'no_catalog' &&
                                            !form.data.catalog_product_id && (
                        ...
                        {step === 2 && (
                            <section className="space-y-6 p-6">
                                    ...
                                    <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
                                        ...
                                        <Field label="Índice de carga">
                                            <input
                                                ...
                                                onChange={(e) => form.setData('load_index', e.target.value)}
                                        ...
                                        <Field label="Cantidad de llantas">
                                            <input
                                                ...
                                                min="1"
                                                max="20"
                                                value={form.data.tire_quantity}
                                                onChange={(e) => form.setData('tire_quantity', e.target.value)}
                                                className={inputCls()}
                                            />
                                        </Field>
                                        ...
                                        <Field label="Lateral">
                                            <input
                                                ...
                                                onChange={(e) => form.setData('sidewall', e.target.value)}
                                                className={inputCls()}
                                            />
                                        </Field>
                                        ...
                                        <Field label="¿Es run flat?">
                                            <select
                                                value={form.data.run_flat}
                                                onChange={(e) => form.setData('run_flat', e.target.value)}
                                                className={inputCls()}
                                            >
                                ...
                                <div className="rounded-2xl border border-zinc-200 bg-white p-4 dark:border-neutral-800 dark:bg-neutral-950">
                                    <div className="text-sm font-semibold text-zinc-900 dark:text-white">Título</div>
                                    <input
                                        type="text"
                                        maxLength="60"
