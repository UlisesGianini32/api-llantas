...
function emptyForm(defaultTimezone) {
    return {
        ...
        timezone: defaultTimezone, active: false, all_day: false, discount_percentage: '10', items: [],
    }
}
...
export default function ScheduledDiscounts({
    ...
    const openEdit = (promotion) => {
        ...
        form.setData({
            ...
            all_day: Boolean(promotion.all_day),
        ...
        <AppShell title="Meli Price Manager">
            ...
            <div className="space-y-6">
                <header className="flex flex-col justify-between gap-4 xl:flex-row xl:items-end">
                    <div><p className="text-xs font-bold uppercase tracking-[0.2em] text-indigo-600">Meli Price Manager</p><h1 className="mt-1 text-3xl font-bold">Promociones programadas</h1><p className="mt-2 text-sm text-slate-500">PRICE_DISCOUNT de Beauty por periodo, horario o día completo y publicación.</p></div>
                ...
                {editor && <section className="rounded-2xl border border-indigo-200 bg-indigo-50/50 p-5 dark:border-indigo-500/20 dark:bg-indigo-500/5">
                    <div className="mb-5 flex flex-col justify-between gap-3 sm:flex-row sm:items-start"><div><h2 className="text-lg font-bold">{editor.mode === 'edit' ? 'Editar promoción' : 'Nueva promoción'}</h2><p className="text-sm text-slate-500">Zona fija: {timezoneLabel(defaultTimezone)}. Fechas DD/MM/AAAA{form.data.all_day ? '; activa durante todo el día.' : ' y horas HH:mm (24 h).'}</p></div><div className="flex flex-wrap gap-2"><button type="submit" form="scheduled-discount-form" disabled={form.processing || !selectedCount} className={primaryButton}>{form.processing ? 'Guardando…' : 'Guardar promoción'}</button><button type="button" onClick={() => setEditor(null)} disabled={form.processing} className={secondaryButton}>Cancelar</button></div></div>
                    <form id="scheduled-discount-form" onSubmit={submit} className="space-y-5">
                        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                            ...
                            {!form.data.all_day && <label><span className="mb-1 block text-sm font-semibold">Hora inicial</span><input value={form.data.starts_at} onChange={(e) => form.setData('starts_at', normalizeClock(e.target.value))} placeholder="20:00" inputMode="numeric" maxLength={5} className={fieldClass} /><ErrorText message={form.errors.starts_at} /></label>}
                            {!form.data.all_day && <label><span className="mb-1 block text-sm font-semibold">Hora final</span><input value={form.data.ends_at} onChange={(e) => form.setData('ends_at', normalizeClock(e.target.value))} placeholder="06:00" inputMode="numeric" maxLength={5} className={fieldClass} /><ErrorText message={form.errors.ends_at} /></label>}
                            ...
                            <div className="grid gap-2 self-end sm:grid-cols-2 xl:col-span-2">
                                <label className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 dark:border-neutral-700 dark:bg-neutral-900"><input type="checkbox" checked={Boolean(form.data.active)} onChange={(e) => form.setData('active', e.target.checked)} /><span className="text-sm font-semibold">Promoción habilitada</span></label>
                                <label className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 dark:border-neutral-700 dark:bg-neutral-900"><input type="checkbox" checked={Boolean(form.data.all_day)} onChange={(e) => form.setData('all_day', e.target.checked)} /><span className="text-sm font-semibold">24 horas</span></label>
                            </div>
                        </div>
