...
export default function InventoryProductShow({ product, physicalStock = 0, reservedStock = 0, availableStock = 0, stockByLocation = [], movements = [], reservations = [] }) {
    return (
        <AppShell title="Producto de almacén">
            ...
            <div className="mx-auto max-w-3xl space-y-6">
                ...
                <div className="grid gap-4 rounded-2xl border border-slate-200 bg-white p-6 sm:grid-cols-2 dark:border-neutral-800 dark:bg-neutral-900">
                    {[['SKU', product.sku], ['Código de barras', product.barcode || '—'], ['Ubicación principal', product.primary_location?.code || 'Sin ubicación'], ['Stock físico', Number(physicalStock || 0)], ['Reservado', Number(reservedStock || 0)], ['Disponible', Number(availableStock || 0)], ['Costo', money(product.cost)], ['Precio Mercado Libre', money(product.price_mercado_libre)], ['Precio Amazon', money(product.price_amazon)], ['Precio estilista / Shopify', money(product.price_stylist)], ['Precio público', money(product.price_public)], ['Estado', product.is_active ? 'Activo' : 'Inactivo']].map(([label, value]) => <div key={label}><dt className="text-xs font-semibold uppercase tracking-wide text-slate-500">{label}</dt><dd className="mt-1 text-slate-900 dark:text-white">{value}</dd></div>)}
                ...
                <section className="rounded-2xl border border-slate-200 bg-white dark:border-neutral-800 dark:bg-neutral-900">
                    ...
                    <div className="divide-y divide-slate-100 dark:divide-neutral-800">
                        {stockByLocation.length === 0 ? <p className="px-5 py-5 text-sm text-slate-500">Sin movimientos ni reservas por ubicación.</p> : stockByLocation.map((row) => <div key={row.inventory_location_id} className="grid grid-cols-4 gap-2 px-5 py-3 text-sm"><span>{row.location?.code || 'Sin ubicación'}</span><span>Físico: {row.physical_stock}</span><span>Reservado: {row.reserved_stock}</span><strong>Disponible: {row.available_stock}</strong></div>)}
                    </div>
                </section>
                <section className="rounded-2xl border border-slate-200 bg-white dark:border-neutral-800 dark:bg-neutral-900">
                    <div className="border-b border-slate-200 px-5 py-4 font-bold dark:border-neutral-800">Reservas activas recientes</div>
                    <div className="divide-y divide-slate-100 dark:divide-neutral-800">
                        {reservations.length === 0 ? <p className="px-5 py-5 text-sm text-slate-500">Sin reservas activas.</p> : reservations.map((reservation) => <div key={reservation.id} className="flex flex-wrap items-center justify-between gap-2 px-5 py-3 text-sm"><span>{reservation.location?.code || 'Global'} · {reservation.reference || 'Sin referencia'}</span><strong>{reservation.quantity}</strong></div>)}
                    </div>
                </section>
                <section className="rounded-2xl border border-slate-200 bg-white dark:border-neutral-800 dark:bg-neutral-900">
