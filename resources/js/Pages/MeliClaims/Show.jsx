...
import ClaimMessageBody from '@/Components/ClaimMessageBody'
...
function Messages({ value, claimId }) { const values = [...listData(value)].sort((a, b) => new Date(b.message_date || b.date_created || 0) - new Date(a.message_date || a.date_created || 0)); return values.length ? <div className="space-y-3">{values.map((m, i) => <article key={i} className="rounded-lg border p-3"><p className="text-sm font-bold">{playerRole(m.sender_role)} → {playerRole(m.receiver_role)} · {date(m.message_date || m.date_created)}</p><ClaimMessageBody value={m.message || m.translated_message || ''} />{m.attachments?.length > 0 && <ul className="mt-2 text-sm text-slate-500">{m.attachments.map((a, j) => { const remote = a.filename || a.file_name; return <li key={j}>Adjunto: {a.original_filename || remote || 'archivo'}{a.type ? ` · ${a.type}` : ''}{a.size ? ` · ${fileSize(a.size)}` : ''}{remote && <> · <a className="font-bold text-indigo-600" href={`/meli-claims/${claimId}/attachments/${encodeURIComponent(remote)}/download`}>Ver / Descargar</a></>}</li> })}</ul>}</article>)}</div> : <Empty>Este reclamo no tiene mensajes disponibles.</Empty> }
