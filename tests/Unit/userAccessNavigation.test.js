...
test('admin retains every existing sidebar section and module', () => {
    ...
    assert.ok(labels.includes('Comparador de llantas'))
    assert.ok(labels.includes('Llantas compuestas'))
    assert.equal(labels.includes('Productos compuestos'), false)
    assert.equal(labels.includes('Llantas agotadas'), false)
    assert.equal(labels.includes('Llantas no actualizadas'), false)
})
